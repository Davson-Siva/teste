...
def realiza_consulta():
    ...
    chamados = consulta_chamados()

    if len(chamados) == 0:
        ultra_debug("Não há nenhuma chamado para realizar")
        return "-1"
    else:
        ultra_debug("VAMOS INICIAR O ATENDIMENTO DOS CHAMADOS")
    ...
    for chamado in chamados:
        motivo_id = chamado[0]
        cpf_cliente = chamado[1]
        clientes_prec_cp = chamado[2]
        ultra_debug("PEGANDO INFORMAÇÕES DO CHAMADO")
        ultra_debug(str(motivo_id))
        ultra_debug(str(cpf_cliente))
        ultra_debug(str(clientes_prec_cp))
        ...
        if respondido == True:
            ...
            if atualiza_chamado == True:
                if senha_eb != "SENHA NAO ENCONTRADO":
                    ...
                    time.sleep(1)
                    if(excluir_senha(clientes_prec_cp)):
                        ultra_debug("prec_cp excluido")
                        ultra_debug("atualizando senha")
                        senha_eb = str(senha_eb)
                        time.sleep(1)
                        cliente_prec_cp = str(clientes_prec_cp)
                        if(atualiza_senha(senha_eb, clientes_prec_cp)):
